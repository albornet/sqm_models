...
def train_decode(wrapp, obj_type, n_objs, im_dims, n_epochs, batch_size, n_batches, init_lr):
  ...
  if not mngr_model.latest_checkpoint:
    print('\nWarning: your reconstruction model is not trained yet.')
  ...
  for e in range(n_epochs):
    ...
    for b in range(n_batches): # batch shape: (batch_s, n_frames) + im_dims
      ...
      dec_loss = wrapp.train_step(tf.stack(batch, axis=1)/255, b, e, optim, labels, -1)
      if b == 0:    
        lr_str = "{:.2e}".format(optim._decayed_lr(tf.float32).numpy())
        print('\nStarting epoch %03i, lr = %s, dec loss = %.3f' % (e, lr_str, dec_loss))
